...
pdfName = './naca_optimization_results_largerfont.pdf'
...
for poly in poly_list:
    ...
    plt.plot(n_design_list, n_iterations_list, '-o', ms=marker_size,
            label=r'RS-QN, $m$ = '+str(n_dofs_list[poly]))

    plt.figure("work_vs_nctl_2",figsize=wide_figsize)
...
for i,n_design in enumerate(WorkvsNDOFS_design_list):
    ...
    plt.figure("work_vs_nstate",figsize=wide_figsize)
    plt.plot(x, y,
...
#                label=r'RS RS-N, $m$ = '+str(n_dofs_list[poly]))
    ...
    plt.figure("work_vs_nctl_3",figsize=wide_figsize)
    plt.plot(n_design_list_more, work_list, '-.o',
            linestyle = linestyle_str[poly],
            color=color_str[newton_color],
...
#print(handles, labels)
lgd = fig_all_cycles.legend(handles, labels, loc='upper right', bbox_to_anchor=(0.98,0.94))
