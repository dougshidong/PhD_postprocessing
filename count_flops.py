...
def numerical_flux(dim):
    ...
    # # Lax-Friedrichs flux
    # flops = 2*analytical_flux(dim) # analytical fluxes
    # flops = flops + 2*dim*(dim+2) # average flux
    # # Eigenvalues
    # eig_flops = dim*2 # velocities
    # eig_flops = eig_flops + dim-1 + dim # velocities squared
    # eig_flops = eig_flops + 4 # pressure
    # eig_flops = eig_flops + 7 # sound
    # eig_flops = eig_flops + 5 # u + sound

    # flops = flops + 2*eig_flops
    # flops = flops + (dim+2) * (dim*2 + 4)
...
def dRdW_vmult_cost(dim,poly):
    ...
    relative_cost = flops / r_flops
    relative_cost = relative_cost / 3.0
...
def d2R_vmult_cost(dim,poly):
    work = 0
    work = work + d2RdWdW_vmult_cost(dim,poly)
    work = work + d2RdWdX_vmult_cost(dim,poly)
    work = work + d2RdXdW_vmult_cost(dim,poly)
    work = work + d2RdXdX_vmult_cost(dim,poly)
    return work
...
def form_d2RdWdW_AD_cost(dim,poly):
    d2RdWdW_cost = reverse_mode_ratio(1) * form_dRdW_AD_cost(dim,poly)
    return d2RdWdW_cost

def form_d2RdWdX_AD_cost(dim,poly):
    d2RdWdX_cost = reverse_mode_ratio(1) * form_dRdX_AD_cost(dim,poly)
    return d2RdWdX_cost

def form_d2RdXdW_AD_cost(dim,poly):
    return d2RdWdX_cost(dim,poly)

def form_d2RdXdX_AD_cost(dim,poly):
    d2RdXdX_cost = reverse_mode_ratio(1) * form_dRdX_AD_cost(dim,poly)
    return d2RdXdX_cost

def form_d2R_AD_cost(dim,poly):
    work = 0
    work = work + form_d2RdWdW_AD_cost(dim, poly)
    work = work + form_d2RdWdX_AD_cost(dim, poly)
    work = work + form_d2RdXdX_AD_cost(dim, poly)
    return work

# def form_d2RdWdW_AD_cost(dim,poly):
#     d2RdWdW_cost = form_dRdW_AD_cost(dim,poly) * form_dRdW_AD_cost(dim,poly)
#     return d2RdWdW_cost
# 
# def form_d2RdWdX_AD_cost(dim,poly):
#     d2RdWdX_cost = form_dRdW_AD_cost(dim,poly) * form_dRdX_AD_cost(dim,poly)
#     return d2RdWdX_cost
# 
# def form_d2RdXdW_AD_cost(dim,poly):
#     return form_d2RdWdX_AD_cost(dim,poly)
# 
# def form_d2RdXdX_AD_cost(dim,poly):
#     d2RdXdX_cost = form_dRdX_AD_cost(dim,poly) * form_dRdX_AD_cost(dim,poly)
#     return d2RdXdX_cost
# 
# def form_d2R_AD_cost(dim,poly):
#     work = 0
#     work = work + form_d2RdWdW_AD_cost(dim, poly)
#     work = work + form_d2RdWdX_AD_cost(dim, poly)
#     work = work + form_d2RdXdW_AD_cost(dim, poly)
#     work = work + form_d2RdXdX_AD_cost(dim, poly)
#     return work
...
for poly in range(0,5):
    ...
    for dim in range(2,4):
        ...
        rel_cost_vmult_d2RdWdW_v = dRdW_vmult_cost(dim,poly)
        ...
        rel_cost_vmult_d2RdWdX_v = dRdX_vmult_cost(dim,poly)
        ...
        rel_cost_vmult_d2RdXdX_v = dRdX_vmult_cost(dim,poly) * dim/(n_state)
        ...
        print ('\n')
        print (rel_cost_AD_d2RdWdW_v)
        print (rel_cost_vmult_d2RdWdW_v)
